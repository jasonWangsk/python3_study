...
def handleEvent(events):
    for d in events['actions']:
        if (d['type'] == 'touch'):
            ...
            monkey.drag(touch_s_x, touch_s_y, touch_e_x, touch_e_y, 0.1, 1)
            print
            'drag ', touch_s_x, touch_s_y, touch_e_x, touch_e_y


def handle_download_event():
    print
    'execute handle_download_event'
    handleEvent(download_event)
    print
    'execute handle_download_event_end'


def handle_share_event():
    print
    'execute handle_share_event'
    handleEvent(share_event)
    print
    'execute handle_share_event end'


def handle_scan_event():
    print
    'execute handle_scan_event'
    handleEvent(scan_event)
    print
    'execute handle_scan_event end'


def handle_del_apk():
    print
    'execute handle_del_apk'
    ...
    print
    'will remove apk pname=', apkPName
    monkey.removePackage(apkPName)
    monkey.sleep(5)
    print
    'execute handle_del_apk end'
